...
class ZWaveNetwork(ZWaveObject):
    ...
    * DriverReset : Sent to indicate when a controller has been reset. This notification is intended to replace the potentially hundreds of notifications representing each value and node removed from the network.
    ...
    def __init__(self, options, log=None, autostart=True, kvals=True):
        ...
        self._manager = libopenzwave.Manager.create()
    ...
    def zwcallback(self, notification):
        ...
        n['valueId'] = {
            ...
            * 'home_id' : v.GetHomeId(),
            * 'node_id' : v.GetNodeId(),
            * 'commandClass' : Manager.COMMAND_CLASS_DESC[v.GetCommandClassId()],
        ...
        :param notification: A dict containing informations about the state of the controller
        :type notification: dict()

        """
        try:
            notify_type = str(notification.getType())[5:]
            logger.debug('zwcallback type=%s' % notify_type)
            if notify_type == self.SIGNAL_DRIVER_FAILED:
                ...
                logger.warning(u'Skipping unhandled notification [%s]', notification)
        except:
            import sys, traceback
    ...
    def _handle_driver_failed(self, notification):
        ...
        :param notification: data sent by the notification
        :type notification: dict()

        dispatcher.send(self.SIGNAL_NETWORK_FAILED, **{'network': self})

        """
        logger.warning(u'Z-Wave Notification DriverFailed : %s', notification)
        self._manager = None
    ...
    def _handle_driver_ready(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification DriverReady : %s', notification)
        self._object_id = notification.getHomeId()
        try:
            node_id = notification.getNodeId()
            controller_node = ZWaveNode(node_id, network=self)
            self._semaphore_nodes.acquire()

            self.nodes = { }
            self.nodes[node_id] = controller_node
            self._controller.node = self.nodes[node_id]
    ...
    def _handle_driver_reset(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification DriverReset : %s', notification)
        try:
            self._semaphore_nodes.acquire()
    ...
    def _handle_driver_removed(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification DriverRemoved : %s', notification)
        try:
            self._semaphore_nodes.acquire()
            self._state = self.STATE_STOPPED
    ...
    def _handle_group(self, notification):
        ...
        dispatcher.send(self.SIGNAL_GROUP, **{'network': self, 'node': self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification Group : %s', notification)
        dispatcher.send(self.SIGNAL_GROUP, \
                **{'network': self, 'node': self.nodes[notification.getNodeId()], 'groupidx': notification.get_group_idx()})
    ...
    def _handle_node(self, node):
        ...
        dispatcher.send(self.SIGNAL_NODE, **{'network': self, 'node':self.nodes[notification.getNodeId()]})
    ...
    def _handle_node_added(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification NodeAdded : %s', notification)
        try:
            node_id = notification.getNodeId()
            node = ZWaveNode(node_id, network=self)
            self._semaphore_nodes.acquire()
            self.nodes[node_id] = node
            dispatcher.send(self.SIGNAL_NODE_ADDED, \
                **{'network': self, 'node': self.nodes[node_id]})
            self._handle_node(self.nodes[node_id])
        finally:
            self._semaphore_nodes.release()
    ...
    def _handle_scene_event(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification SceneEvent : %s', notification)
        dispatcher.send(self.SIGNAL_SCENE_EVENT, \
            **{'network': self, 'node': self.nodes[notification.getNodeId()],
               'scene_id': notification.get_scene_id()})
    ...
    def _handle_node_event(self, notification):
        ...
        dispatcher.send(self.SIGNAL_NODE_EVENT, **{'network': self, 'node': self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification NodeEvent : %s', notification)
        dispatcher.send(self.SIGNAL_NODE_EVENT,
                        **{'network': self, 'node': self.nodes[notification.getNodeId()], 'value': notification.get_event()})

    def _handle_node_naming(self, notification):
        """
        One of the node names has changed (name, manufacturer, product).

        dispatcher.send(self.SIGNAL_NODE_NAMING, **{'network': self, 'node': self.nodes[notification.getNodeId()]})
        dispatcher.send(self.SIGNAL_NODE, **{'network': self, 'node':self.nodes[notification.getNodeId()]})
        ...
        """
        logger.debug(u'Z-Wave Notification NodeNaming : %s', notification)
        node_id = notification.getNodeId()
    ...
    def _handle_node_new(self, notification):
        ...
        """
        logger.debug('Z-Wave Notification NodeNew : %s', notification)
        node_id = notification.getNodeId()
        dispatcher.send(self.SIGNAL_NODE_NEW, \
            **{'network': self, 'node_id': node_id})
    ...
    def _handle_node_protocol_info(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification NodeProtocolInfo : %s', notification)
        node_id = notification.getNodeId()
    ...
    def _handle_node_removed(self, notification):
        ...
        dispatcher.send(self.SIGNAL_NODE_REMOVED, **{'network': self, 'node_id': notification.getNodeId()})
        dispatcher.send(self.SIGNAL_NODE, **{'network': self, 'node':self.nodes[notification.getNodeId()]})
        ...
        """
        logger.debug(u'Z-Wave Notification NodeRemoved : %s', notification)
        try:
            node_id = notification.getNodeId()
            self._semaphore_nodes.acquire()
    ...
    def _handle_essential_node_queries_complete(self, notification):
        ...
        dispatcher.send(self.SIGNAL_ESSENTIAL_NODE_QUERIES_COMPLETE, **{'network': self, 'node': self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification EssentialNodeQueriesComplete : %s', notification)
    ...
    def _handle_node_queries_complete(self, notification):
        ...
        dispatcher.send(self.SIGNAL_NODE_QUERIES_COMPLETE, **{'network': self, 'node': self.nodes[notification.getNodeId()]})
        dispatcher.send(self.SIGNAL_NODE, **{'network': self, 'node':self.nodes[notification.getNodeId()]})
        ...
        """
        logger.debug(u'Z-Wave Notification NodeQueriesComplete : %s', notification)
        #the query stage are now completed, set the flag is ready to operate
        node_id = notification.getNodeId()
        self.nodes[node_id].is_ready = True
    ...
    def _handle_all_nodes_queried(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification AllNodesQueried : %s', notification)
    ...
    def _handle_all_nodes_queried_some_dead(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification AllNodesQueriedSomeDead : %s', notification)
        self._state = self.STATE_READY
        dispatcher.send(self.SIGNAL_NETWORK_READY, **{'network': self})
    ...
    def _handle_awake_nodes_queried(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification AwakeNodesQueried : %s', notification)
        self._object_id = notification.getHomeId()
        try:
    ...
    def _handle_polling_disabled(self, notification):
        ...
        dispatcher.send(self.SIGNAL_POLLING_DISABLED, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification PollingDisabled : %s', notification)
    ...
    def _handle_polling_enabled(self, notification):
        ...
        dispatcher.send(self.SIGNAL_POLLING_ENABLED, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification PollingEnabled : %s', notification)
    ...
    def _handle_create_button(self, notification):
        ...
        dispatcher.send(self.SIGNAL_CREATE_BUTTON, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification CreateButton : %s', notification)
    ...
    def _handle_delete_button(self, notification):
        ...
        dispatcher.send(self.SIGNAL_DELETE_BUTTON, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification DeleteButton : %s', notification)
    ...
    def _handle_button_on(self, notification):
        ...
        dispatcher.send(self.SIGNAL_BUTTON_ON, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification ButtonOn : %s', notification)
    ...
    def _handle_button_off(self, notification):
        ...
        dispatcher.send(self.SIGNAL_BUTTON_OFF, **{'network': self, 'node' : self.nodes[notification.getNodeId()]})

        :param notification: data sent by the notification
        :type notification: dict()

        """
        logger.debug(u'Z-Wave Notification ButtonOff : %s', notification)
    ...
    def _handle_value(self, node=None, value=None):
        ...
        :param node_id: the id of the node who hold the value
        :type node_id: int
    ...
    def _handle_value_added(self, notification):
        ...
        dispatcher.send(self.SIGNAL_VALUE_ADDED, \
            **{'network': self, 'node' : self.nodes[notification.getNodeId()], \
                'value' : self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]})
        dispatcher.send(self.SIGNAL_VALUE, **{'network': self, 'node' : node, 'value' : value})
        ...
        """
        logger.debug(u'Z-Wave Notification ValueAdded : %s', notification)
        node_id = notification.getNodeId()
        self.nodes[node_id].add_value(notification.getValueID().getId())
        dispatcher.send(self.SIGNAL_VALUE_ADDED, \
            **{'network': self, \
               'node' : self.nodes[notification.getNodeId()], \
               'value' : self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]})
        self._handle_value(node=self.nodes[notification.getNodeId()], value=self.nodes[notification.getNodeId()].values[notification.getValueID().getId()])
    ...
    def _handle_value_changed(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification ValueChanged : %s', notification)
        if notification.getNodeId() not in self.nodes:
            logger.warning('Z-Wave Notification ValueChanged (%s) for an unknown node %s', notification['valueId'], notification.getNodeId())
            return False
        self.nodes[notification.getNodeId()].change_value(notification.getValueID().getId())
        dispatcher.send(self.SIGNAL_VALUE_CHANGED, \
            **{'network': self, 'node' : self.nodes[notification.getNodeId()], \
                'value' : self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]})
        self._handle_value(node=self.nodes[notification.getNodeId()], value=self.nodes[notification.getNodeId()].values[notification.getValueID().getId()])
    ...
    def _handle_value_refreshed(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification ValueRefreshed : %s', notification)
        if notification.getNodeId() not in self.nodes:
            logger.warning('Z-Wave Notification ValueRefreshed (%s) for an unknown node %s', notification.getValueID(), notification.getNodeId())
            return False
        self.nodes[notification.getNodeId()].refresh_value(notification.getValueID().getId())
        dispatcher.send(self.SIGNAL_VALUE_REFRESHED, \
            **{'network': self, 'node' : self.nodes[notification.getNodeId()], \
                'value' : self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]})
        self._handle_value(node=self.nodes[notification.getNodeId()], value=self.nodes[notification.getNodeId()].values[notification.getValueID().getId()])
    ...
    def _handle_value_removed(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification ValueRemoved : %s', notification)
        if notification.getNodeId() not in self.nodes:
            logger.warning(u'Z-Wave Notification ValueRemoved (%s) for an unknown node %s', notification.getValueID(), notification.getNodeId())
            return False
        if notification.getValueID().getId() in self.nodes[notification.getNodeId()].values:
            logger.warning(u'Z-Wave Notification ValueRemoved for an unknown value (%s) on node %s', notification.getValueID(), notification.getNodeId())
            dispatcher.send(self.SIGNAL_VALUE_REMOVED, \
                **{'network': self, 'node' : self.nodes[notification.getNodeId()], \
                    'value' : None, 'valueId' : notification.getValueID().getId()})
            return False
        val = self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]
        if self.nodes[notification.getNodeId()].remove_value(notification.getValueID().getId()):
            dispatcher.send(self.SIGNAL_VALUE_REMOVED, \
                **{'network': self, 'node' : self.nodes[notification.getNodeId()], \
                    'value' : val, 'valueId' : notification.getValueID().getId()})
            #self._handle_value(node=self.nodes[notification.getNodeId()], value=val)
        if notification.getNodeId() in self.nodes and notification.getValueID().getId() in self.nodes[notification.getNodeId()].values:
            del self.nodes[notification.getNodeId()].values[notification.getValueID().getId()]
        return True
    ...
    def _handle_notification(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification : %s', notification)
        dispatcher.send(self.SIGNAL_NOTIFICATION, \
            **{'network': self, 'notification': notification})
    ...
    def _handle_controller_command(self, notification):
        ...
        dispatcher.send(self.SIGNAL_CONTROLLER_WAITING, \
            **{'network': self, 'controller': self.controller,
               'state_int': notification['controllerStateInt'], 'state': notification['controllerState'], 'state_full': notification['controllerStateDoc'],
               })
        ...
        dispatcher.send(self.SIGNAL_CONTROLLER_COMMAND, \
            **{'network': self, 'controller': self.controller,
               'node':self.nodes[notification.getNodeId()] if notification.getNodeId() in self.nodes else None, 'node_id' : notification.getNodeId(),
               'state_int': notification['controllerStateInt'], 'state': notification['controllerState'], 'state_full': notification['controllerStateDoc'],
               'error_int': notification['controllerErrorInt'], 'error': notification['controllerError'], 'error_full': notification['controllerErrorDoc'],
               })
        ...
        """
        self._controller._handle_controller_command(notification)
    ...
    def _handle_msg_complete(self, notification):
        ...
        """
        logger.debug(u'Z-Wave Notification MsgComplete : %s', notification)

...
def imports(args):
    ...
    if args.output == 'txt':
        ...
        import libopenzwave
        print("Try to import libopenzwave.LogLevel")
        from libopenzwave import LogLevel
        print("Try to get options")
        options = libopenzwave.Options.create(user_path=".", cmd_line="--logging false")
        options.lock()
        time.sleep(0.5)
        print("Try to get manager")
        manager = libopenzwave.Manager.create()
...
def init_device(args):
    ...
    if args.output == 'txt':
        ...
        import libopenzwave
        print("Try to get options")
        options = libopenzwave.Options.create(config_path=args.config_path, user_path=args.user_path, cmd_line="--logging true")
        options.lock()
        time.sleep(1.0)
        print("Try to get manager")
        manager = libopenzwave.Manager.create()
        ...
        import libopenzwave
        options = libopenzwave.Options.create(config_path=args.config_path, user_path=args.user_path, cmd_line="--logging false")
        options.lock()
        time.sleep(1.0)
        manager = libopenzwave.Manager.create()
        manager.addWatcher(zwcallback)
